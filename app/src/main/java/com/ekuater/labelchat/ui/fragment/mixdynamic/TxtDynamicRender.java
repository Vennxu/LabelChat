 ...
 * @author LinYong
 */
public class TxtDynamicRender implements DynamicRender, View.OnClickListener {
    ...
    public TxtDynamicRender(Context context, boolean isLoadComment, StoryDynamicListener listener) {
    ...
    public View getView(LayoutInflater inflater, ViewGroup parent) {
        ...
        View convertView = inflater.inflate(R.layout.layout_story_detail_descript, parent, false);
        ...
        holder.story_content = (ShowContentTextView) convertView.findViewById(R.id.descript_content);
        holder.story_content.setAutoLinkMask(Linkify.ALL);
        holder.story_content_image = (ImageView) convertView.findViewById(R.id.descript_image);
        holder.story_image_list = (LinearLayout) convertView.findViewById(R.id.descript_image_list);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.descript_image:
                if(story.getImages().length > 1) {
                    listener.onStoryMoreImageClick(story.getImages(), Integer.parseInt(v.getTag().toString()));
                }else{
                    listener.onStoryMoreImageClick(story.getImages(), 0);
                }
                break;
    ...
    private void bindStory(LabelStory story) {
        ...
        holder.story_content_image.setTag(0);
...
//            }
            if (imageThumbs.length > 1) {
                holder.story_image_list.setVisibility(View.VISIBLE);
                holder.story_image_list.removeAllViews();
                loadingImageList(imageThumbs);
            } else {
                holder.story_image_list.setVisibility(View.GONE);
            }
            holder.story_content_image.setScaleType(ImageView.ScaleType.CENTER_CROP);
        } else {
    ...
    private void loadingImageList(String thumbImage[]) {
        int margin = MiscUtils.dp2px(context, 10);
        int weight = (display.getWidth() - (margin * 8)) / 5;
        for (int i = 0; i < thumbImage.length; i++) {
            final int position = i;
            final String imageUrl = thumbImage[position];
            View view = inflater.inflate(R.layout.descript_image_item, holder.story_image_list, false);
            ...
            imageBg.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    MiscUtils.showLabelStoryImageThumb(avatarManager,
                            imageUrl, holder.story_content_image,
                            R.drawable.pic_loading);
                    holder.story_content_image.setTag(position);
            ...
            MiscUtils.showLabelStoryImageThumb(avatarManager,
                    ...
                    R.drawable.pic_loading);
            holder.story_image_list.addView(view);
        }
    }
        ...
        for (int i = 0; i < commentses.length; ++i) {
            ...
            if (stranger != null) {
                ...
                if (title != null && title.length() > 0) {
                    ...
                    groupName.setText(stranger.getNickname());
                }
    ...
    private void loadPriseUser(UserPraise[] users) {
        for (int i = 0; i < users.length; ++i) {
            ...
            final UserPraise userPraise = users[i];
            if (userPraise != null) {
                MiscUtils.showAvatarThumb(avatarManager, users[i].getmPraiseUserAvatarThumb(), view, R.drawable.contact_single);
            } else {
    ...
    private static class ViewHolder {
        ...
        public ShowContentTextView story_content;
        ...
        public LinearLayout story_image_list;
    }
}
